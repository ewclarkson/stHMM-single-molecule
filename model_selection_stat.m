...
% assume D1 > D2
tic
nRep = 14; % try this many values of k12,k21
...
nStat = 30; % number of analyses of similar datasets
...
% logZcell_error_Kinz = cell(1,nRep); % hold estimates of log-evidence
...
for i = 1:nRep
    mProb(i) = exp(logZarr_CTHMM(i)-util.logsumexp2(logZarr_CTHMM(i),logZarr_DTHMM(i)));
...
p.EdgeColor = 'none'; 
hold on
plot(x,mProb,'Color',[0 0.4470 0.7410],'LineWidth',1.5)

% legend('discrete', 'continuous', 'ground truth')
ylabel('stHMM probability')
ylim([0.5,1])
xlabel('\nu_1=\nu_2')
xticks(x)
xlim([0,x(end)+2])
...
f1 = openfig('figures/model_probabilities_D1=10_2025-12-09.fig');
ax1 = findall(f1,'type','axes');
% ax1.XTick = k12vec*tau;
% ax1.XScale = 'log';

% f2 = openfig('figures/model_probabilities_D1=3_2025-06-24.fig');
f2 = openfig('figures/model_probabilities_D1=3_2025-12-12.fig');
% ylim([0,1])
...
text(0.01,0.98,ax1,{"\bf{a}"},'FontSize',12,'units','normalized');
% text(0.07,0.97,ax1,{"Well-separated diffusion constants"},'FontSize',10,'units','normalized');
text(0.1,0.97,ax1,{"Well-separated diffusion constants"},'FontSize',9,'units','normalized');
legend('stHMM','HMM','one-state')

% make a zoom-in inset
axInset = axes('Position',[0.2560 0.3838 0.1100 0.1100]);
fInset = openfig('figures/model_probabilities_D1=10_2025-12-09.fig','invisible');
axInset.YScale = 'log';
ch = findall(fInset.CurrentAxes);
ch(ch == fInset.CurrentAxes) = [];
% ch(1).XData = k12vec*tau;
ch = ch(1);
copyobj(ch,axInset)
% axInset.XTick = x;
axInset.XTick = x(2:4:end); % sparse labelling
% axInset.XTickLabel = lblCell;
axInset.XTickLabel = {lblCell{2:4:end}}; % sparse labelling

% include the second plot
ax2c = copyobj(ax2,tl);
ax2c.Layout.Tile = 2;
ax2c.Layout.TileSpan = [1 1];
text(0.01,0.97,ax2,{"\bf{b}"},'Parent',ax2c,'FontSize',12,'units','normalized');
% text(0.07,0.97,ax2,{"Less-separated diffusion constants"},'Parent',ax2c,'FontSize',10,'units','normalized');
text(0.1,0.97,ax2,{"Less-separated diffusion constants"},'Parent',ax2c,'FontSize',9,'units','normalized');
ylim([0,1.1])

% make another zoom-in inset
axInset = axes('Position',[0.7000 0.3838 0.1100 0.1100]);
fInset = openfig('figures/model_probabilities_D1=3_2025-12-12.fig','invisible');
axInset.YScale = 'log';
ch = findall(fInset.CurrentAxes);
ch(ch == fInset.CurrentAxes) = [];
% ch(1).XData = k12vec*tau;
