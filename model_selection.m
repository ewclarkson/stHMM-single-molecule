...
sigmaE = 0; %0.010; % localisation error std
% M = 100; % number of sub-time intervals for trajectory generation
% mC = 1; % motion blur related. 1: no motion blur
Rmb = 0; %1/6; % motion blur coefficient. 1/6: continous shutter mode. 0: no motion blur.
...
% logBF = logZarr_CTHMM-logZarr_DTHMM;
% logBF_upper = logBF+logZarr_error_CTHMM+logZarr_error_DTHMM;
% logBF_lower = logBF-logZarr_error_CTHMM-logZarr_error_DTHMM;
% 
% xconf = [x x(end:-1:1)];
...
% p.EdgeColor = 'none'; 
% hold on
% plot(x,logBF,'Color',[0 0.4470 0.7410],'LineWidth',1.5)
% 
...
% legend('discrete', 'continuous', 'ground truth')
ylabel('stHMM model probability')
xlabel('\nu_1=\nu_2')
xticks(x)
xlim([0,x(end)+2])
...
for i = 1:4 % do for every parameter
    ...
    p2.EdgeColor = 'none'; 
    hold on
    plot(x,y_iter_DTHMM,'-.','Color',[0 0.4470-0.2 0.7410-0.2],'LineWidth',2.5)
    hold on
    ...
    p3.FaceColor = [0.8 0.4 0.0];
    p3.FaceAlpha = 0.7;
    p3.EdgeColor = 'none'; 
    hold on
    plot(x,y_iter_CTHMM,'Color',[0.3500 0.20 0.2],'LineWidth',1.2)
    hold on
    ...
    if i == 1
        legend([p3,p2,p4],{'stHMM','HMM','simulated'},'Location','northwest',...
               'AutoUpdate','off')
