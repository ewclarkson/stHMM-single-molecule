...
%% include also text for each row
...
f = figure('Position',[500 100 600 450]);
tl = tiledlayout(3,21,'TileSpacing','compact','Padding','compact');
MyTileSpan = [1 10];
% plot 1
ax1c = copyobj(ax1,tl);
ax1c(1).Layout.Tile = 2;
ax1c.Layout.TileSpan = MyTileSpan;
text(0.01,0.95,{"\bf{a}"},'FontSize',12,'units','normalized');
% text(-0.25,0.4,{"slow"},'FontSize',9,'units','normalized','Rotation',90);
text(-0.22,0.4,{"slow"},'FontSize',9,'units','normalized','Rotation',90);
% text(-0.2,0.3,{"switching"},'FontSize',9,'units','normalized','Rotation',90);
text(-0.17,0.3,{"switching"},'FontSize',9,'units','normalized','Rotation',90);


% plot 2
ax2c = copyobj(ax2,tl);
ax2c(1).Layout.Tile = 12;
ax2c.Layout.TileSpan = MyTileSpan;
text(0.01,0.92,ax2,{"\bf{b}"},'Parent',ax2c,'FontSize',12,'units','normalized');

% plot 3
ax3c = copyobj(ax3,tl);
ax3c(1).Layout.Tile = 23;
ax3c.Layout.TileSpan = MyTileSpan;
text(0.01,0.95,ax3,{"\bf{c}"},'Parent',ax3c,'FontSize',12,'units','normalized');
text(-0.22,0.2,{"intermediate"},'Parent',ax3c,'FontSize',9,'units','normalized','Rotation',90);
text(-0.17,0.3,{"switching"},'Parent',ax3c,'FontSize',9,'units','normalized','Rotation',90);

% plot 4
ax4c = copyobj(ax4,tl);
ax4c(1).Layout.Tile = 33;
ax4c.Layout.TileSpan = MyTileSpan;
text(0.01,0.92,ax4,{"\bf{d}"},'Parent',ax4c,'FontSize',12,'units','normalized');

% plot 5
ax5c = copyobj(ax5,tl);
ax5c(1).Layout.Tile = 44;
ax5c.Layout.TileSpan = MyTileSpan;
text(0.01,0.95,ax5,{"\bf{e}"},'Parent',ax5c,'FontSize',12,'units','normalized');
text(-0.22,0.4,{"fast"},'Parent',ax5c,'FontSize',9,'units','normalized','Rotation',90);
text(-0.17,0.3,{"switching"},'Parent',ax5c,'FontSize',9,'units','normalized','Rotation',90);

% plot 6
ax6c = copyobj(ax6,tl);
ax6c(1).Layout.Tile = 54;
ax6c.Layout.TileSpan = MyTileSpan;
text(0.01,0.92,ax6,{"\bf{f}"},'Parent',ax6c,'FontSize',12,'units','normalized');
