...
sigmaE = 0.015; % localisation error std
% M = 100; % number of sub-time intervals for trajectory generation
% mC = 1; % motion blur related. 1: no motion blur
Rmb = 1/6; % motion blur coefficient. 1/6: continous shutter mode. 0: no motion blur.
...
for i = 1:nRep
    ...
    data = cell(1,nTraj);
    for k = 1:nTraj
        % data{k} = util.generatetrajectory(k12vec(i),k21vec(i),D1,D2,tau,nSteps); % no noise
        % data{k} = util.generatetrajectory(nSteps, tau, 100, D1, D2, k12vec(i), k21vec(i));
        data{k} =  util.generatetrajectory_noisy(nSteps, tau, 100, 100, D1, D2, k12vec(i), k21vec(i), sigmaE);
        % data{k} = util.noisyBrownian2state2D(nSteps, tau, 100, 100, D1, D2, k12vec(i), k21vec(i), sigmaE); % with noise
    end
...
for i = 1:4 % do for every parameter
    ...
    % [lb_DTHMM,ub_DTHMM] = confidenceinterval(binCentra,binCounts,alpha,alpha);
    % yconf = [y_iter_DTHMM+err_iter_DTHMM y_iter_DTHMM(end:-1:1)-err_iter_DTHMM(end:-1:1)];
    ...
    % err_iter_CTHMM = err_CTHMM(:,i)';
    u_err_iter_CTHMM = ubounds_CTHMM(:,i)';
    l_err_iter_CTHMM = lbounds_CTHMM(:,i)';         
    % yconf = [y_iter_CTHMM+err_iter_CTHMM y_iter_CTHMM(end:-1:1)-err_iter_CTHMM(end:-1:1)];
    yconf = [u_err_iter_CTHMM l_err_iter_CTHMM(end:-1:1)];
    p3 = fill(xconf,yconf,'red');
    p3.FaceColor = [0.8500 0.1250 0.080];
    p3.FaceAlpha = 0.6;
    p3.EdgeColor = 'none'; 
    hold on
    plot(x,y_iter_CTHMM,'Color',[0.5500 0.10 0.0],'LineWidth',1.2)
    hold on

    if i == 1
        legend([p3,p1,p2],{'stHMM','BIASD','HMM'},'Location','northwest',...
               'AutoUpdate','off')
    end
