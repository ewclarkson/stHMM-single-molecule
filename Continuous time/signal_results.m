%% Compare DTHMM,CTHMM and Kinz for parameter inference from signal data
...
mu2 = 4; % mean of state 2
sigma = 1; % std of both states
mu1vec = linspace(mu1,mu1,nRep);
mu2vec = linspace(mu2,mu2,nRep);
k12_min_gen = 2;
k12_max_gen = 400;
k21_min_gen = 2;
k21_max_gen = 400;
...
StopRatio = 1E-4; % stop criterion

% data parameters
nSteps = 200; % trajectory length
nTraj = 10; % number of trajectories

% experimental parameters
tau = 0.005; % sampling time
exPars = {'tau', tau; 'sigma', sigma}; % defines experimental parameters that enter models
...
for i = 1:nRep
    ...
    data = cell(1,nTraj);
    for k = 1:nTraj
        data{k} = util.generatesignal(nSteps,tau,100,100,mu1,mu2,sigma,sigma,k12vec(i),k21vec(i)); % no noise
    end
     
    % set priors
    % mu1_max = 20; % known upper limit of D1
    % mu1_min = 2; % known lower limit of D1
    % mu2_max = 8; % known upper limit of D2
    % mu2_min = 0.1; % known lower limit of D2
    % k12_min = 1E-3;
    % k12_max = 3*k12_max_gen;
    % k21_min = 1E-3;
    % k21_max = 3*k21_max_gen;
    % priorLimits = [mu1_min,mu1_max;mu2_min,mu2_max;k12_min,k12_max;k21_min,k21_max];
    % priorPars = {'lognormal',log(mu1),log(1.5); 'lognormal',log(mu2),log(1.5); 'lognormal',log(k12vec(i)),log(5.5); 'lognormal',log(k21vec(i)),log(5.5)};
    priorPars = {'lognormal',log(mu1),log(1.5); 'lognormal',log(mu2),log(1.5); 'lognormal',log(200),log(3.5); 'lognormal',log(200),log(3.5)}; 
    % NOTE: k12,k21-priors should be set equal for all data sets

    % nested sampling with DT-HMM
    [finalSeq_DTHMM, thetaMLE_DTHMM, logZ_DTHMM] = util.nestedsampling(nLive, StopRatio, priorPars, exPars, data, @util.logl_DTHMM_signal);
    
    % run nested sampling with Kinz
    [finalSeq_Kinz,thetaMLE_Kinz,logZ_Kinz] = util.nestedsampling(nLive, StopRatio, priorPars, exPars, data, @util.logl_Kinz_signal);

    % run nested sampling with CT-HMM
    [finalSeq_CTHMM,thetaMLE_CTHMM,logZ_CTHMM] = util.nestedsampling(nLive, StopRatio, priorPars, exPars, data, @util.logl_CTHMM_signal);

    % Zmat(i,j) = logZ_CTHMM-logZ_DTHMM; % logarithmized Bayes factor
    ...
    % Zmat_error(i,j) = logZ_error_das + logZ_error_our;
    ...
    % NSseq_DTHMM{i} = finalSeq_DTHMM;
    % NSseq_CTHMM{i} = finalSeq_CTHMM;
    ...
    % nBins_DTHMM_cell{i} = nBins_DTHMM;
    % nBins_CTHMM_cell{i} = nBins_CTHMM;
    % NSpos_DTHMM_cell{i} = NSpos_DTHMM;
    % NSpos_CTHMM_cell{i} = NSpos_CTHMM;
    % w_DTHMM_cell{i} = w_DTHMM;
    % w_CTHMM_cell{i} = w_CTHMM;
...
x = 1:10:10*nRep;
paramCell = {mu1vec,mu2vec,k12vec,k21vec};
labelCell = {'\mu_1 (a.u.)','\mu_2 (a.u.)','k_{12} (1/s)','k_{21} (1/s)'};
...
for i = 1:4 % do for every parameter
    ...
    p2.EdgeColor = 'none'; 
    hold on
    plot(x,y_iter_DTHMM,'-.','Color',[0 0.4470 0.7410]-[0 0.2 0.2],'LineWidth',2.5)
    hold on
    ...
    % p3.FaceColor = [0.8500 0.3250 0.0980];
    p3.FaceColor = [0.85 0.1250 0.080];
    p3.FaceAlpha = 0.6;
    p3.EdgeColor = 'none';
    
    hold on
    plot(x,y_iter_CTHMM,'Color',[0.5500 0.1 0.0],'LineWidth',1.2)
    
    hold on
    if i == 1
        legend([p3,p1,p2],{'stHMM','BIASD','HMM'},'Location','southwest',...
               'AutoUpdate','off')
    end

    % ground truth
    plot(x,paramCell{i},'--k','LineWidth',1.5);
...
paramCell = {mu1vec,mu2vec,k12vec,k21vec};
xlabelCell = {'\mu_1 (a.u.)','\mu_2 (a.u.)','k_{12} (1/ms)','k_{21} (1/ms)'};
ylabelCell = {'P(\mu_1|O)','P(\mu_2|O)','P(k_{12}|O)','P(k_{21}|O)'};
