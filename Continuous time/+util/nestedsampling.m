function [deadPoints,thetaMLE,logZ] = nestedsampling(nLive, stopRatio, priorPars, exPars, data, loglfun)
    ...
    % 
    %     livePoints(idxPoint).logL = loglfun(pos, exPars, data); % compute and store likelihood
    % end
    ...
    while logZratio > logStopRatio % do until the stop criterion is met, i.e. until Z has been calculated
        ...
        % Store dead point
        % u = livePoints(ind_worst).upos;
        % y = zeros(1,4);
        % for idx = 1:4
        %     y(idx) = unifinv(u(idx),priorPars(idx,1),priorPars(idx,2)); % transform position to parameter space
        % end
