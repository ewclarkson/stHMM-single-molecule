function enslogL = logl_CTHMM_signal(paramArr, exPars, data)
    % 
    % Continuous-time likelihood calculation with a forward algorithm.
    % Calculate the unknown emission mean parameters from signal data.
    % 
    % assume that mu1 > mu2 and that sigma1=sigma2=sigma.
    % 
    % Input:
...
%     if  D1 <= 0 || D2 <= 0 || D1 <= D2 || k12 <= 0 || k21 <= 0 % forbidden inputs
    if mu1 <= mu2 || p12 < 0 || p12 >= 1 || p21 < 0 || p21 >= 1 % these inputs are forbidden
    ...
    %     enslogL = -Inf; % forbid very high rates
        ...
        for j = 1:length(data)
            ...
            y = data{j}; % current trajectory
            N = length(y); % number of displacements in the track

                % ----------- define functions for later computation -----------
        
            % for 1 --> 1, with at least two switches
            L_switch_same_1 = @(w,y) 1./sqrt(2*pi*sigma^2).*exp(-(y-(w*mu1+(1-w)*mu2)).^2./(2*sigma^2))*...
    ...
    % 
            ...
            L_switch_same_2 = @(w,y) 1./sqrt(2*pi*sigma^2).*exp(-(y-(w*mu1+(1-w)*mu2)).^2./(2*sigma^2))*...
    ...
    % 
            ...
            L_switch_opp = @(w,delta) 1./sqrt(2*pi*sigma^2).*exp(-(y-(w*mu1+(1-w)*mu2)).^2./(2*sigma^2))*...
            tau.*exp(-tau*(k12*w+k21*(1-w))).*besseli(0,2*tau*sqrt(k12*k21*w.*(1-w)));
    
            logLstay_1 = -tau*k12-1/2*log(2*pi*sigma^2)-(y-mu1).^2/(2*sigma^2); % for 1 --> 1
            logLstay_2 = -tau*k21-1/2*log(2*pi*sigma^2)-(y-mu2).^2/(2*sigma^2); % for 2 --> 2
            ...
            % for 1 --> 1, with at least two switches
            logLsame_w1 = integral(@(w) L_switch_same_1(w,y), 0,1, 'ArrayValued',true,...
                'AbsTol',epsilon_abs,'RelTol',epsilon_rel);
            logLsame_w1 = log(logLsame_w1);
    
            % for 2 --> 2, with at least two switches
            logLsame_w2 = integral(@(w) L_switch_same_2(w,y), 0,1, 'ArrayValued',true,...
                'AbsTol',epsilon_abs,'RelTol',epsilon_rel);
            logLsame_w2 = log(logLsame_w2);
    
            % for at least one switch
            logLopp = integral(@(w) L_switch_opp(w,y), 0,1, 'ArrayValued',true,...
                'AbsTol',epsilon_abs,'RelTol',epsilon_rel);
