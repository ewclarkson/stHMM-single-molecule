function enslogL = logl_DTHMM_signal(paramArr, exPars, data)
    % 
...
%     if  D1 <= 0 || D2 <= 0 || D1 <= D2 || k12 <= 0 || k21 <= 0 % forbidden inputs
    ...
    %     enslogL = -Inf; % ignore very high rates
        ...
        logL_func = @(y,mu) -1/2*log(2*pi*sigma^2)-(y-mu).^2/(2*sigma^2); % the likelihood for every individual displacement
        % logL_func = @(disp_j, D_i, deltaT) -log(4*pi*D_i*deltaT*(1-2*R)+2*sigmaE^2)-disp_j.^2/(4*D_i*deltaT*(1-2*R)+2*sigmaE^2); 
        % the likelihood for every individual displacement
    
        for i = 1:length(data) % do for every trajectory

            y = data{i}; % current trajectory
            N = length(y); % number of displacements

            % ----------- compute log-likelihoods -----------
    
            likeli_1 = logL_func(y, mu1); % emission probabilities all track displacements, given D1
            likeli_2 = logL_func(y, mu2); % emission probabilities all track displacements, given D2
