...
function displacements = generatetrajectory_noisy(nSteps, tau, M, mC, D1, D2, k12, k21, sigmaE)
...
%     % Displacement which include both motion blur and localization noise 
    DxMBLocNoise = xMBLocNoise(2:nPos) - xMBLocNoise(1:nPos-1); % x-displacement
    DyMBLocNoise = yMBLocNoise(2:nPos) - yMBLocNoise(1:nPos-1); % y-displacement

    displacements = sqrt(DxMBLocNoise.^2 + DyMBLocNoise.^2); % 2D displacement
